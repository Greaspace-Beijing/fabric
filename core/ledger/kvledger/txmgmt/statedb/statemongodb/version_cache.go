...
func (c *versionsCache) getVersion(ns, key string) (*version.Height, bool) {
	logger.Debugf("version_cache.go getVersion: ns : %s  key : %s", key, ns)
	ver, ok := c.vers[ns][key]
	if ok {
		logger.Debugf("version_cache.go getVersion: ver : %s", ver)
		return ver, true
	}
	return nil, false
}
...
func (c *versionsCache) setVerAndRev(ns, key string, ver *version.Height, rev string) {
	logger.Debugf("version_cache.go setVerAndRev: ns : %s  key : %s  ver : %s  rev : %s", key, ns, ver, rev)
	_, ok := c.vers[ns]
	if !ok {
