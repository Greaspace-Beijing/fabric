...
var _ = Describe("MongoDB Runner", func() {
	var (
		dockerServer *ghttp.Server
		mongoServer  *ghttp.Server
	...
	BeforeEach(func() {
		mongoServer = ghttp.NewServer()
		mongoServer.Writer = GinkgoWriter
		mongoServer.AppendHandlers(
			ghttp.RespondWith(http.StatusServiceUnavailable, "service unavailable"),
			ghttp.RespondWith(http.StatusServiceUnavailable, "service unavailable"),
			ghttp.RespondWith(http.StatusOK, "ready"),
		)

		mongoAddr := mongoServer.Addr()
		mongoHost, mongoPort, err := net.SplitHostPort(mongoAddr)
	...
	It("starts and stops a docker container with the specified image", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		mongoDB.Client = nil
		mongoDB.StartTimeout = 0
		mongoDB.Name = containerName

		By("starting mongo DB")
		process = ifrit.Invoke(mongoDB)
		...
		Expect(container.Config.Image).To(Equal("mongo:latest"))
		Expect(container.ID).To(Equal(mongoDB.ContainerID()))
		portBindings := container.NetworkSettings.Ports[docker.Port("27017/tcp")]
		Expect(portBindings).To(HaveLen(1))
		Expect(mongoDB.HostAddress()).To(Equal(net.JoinHostPort(portBindings[0].HostIP, portBindings[0].HostPort)))
		Expect(mongoDB.ContainerAddress()).To(Equal(net.JoinHostPort(container.NetworkSettings.IPAddress, "27017")))

		By("accessing the mongo DB server")
	...
	Context("when a host port is provided", func() {
		...
		It("exposes mongo on the specified port", func() {
			err := mongoDB.Start()
			Expect(err).NotTo(HaveOccurred())
			err = mongoDB.Stop()
			Expect(err).NotTo(HaveOccurred())
	...
	Context("when the container endpoint becomes available before the host", func() {
		BeforeEach(func() {
			mongoHost, mongoPort, err := net.SplitHostPort(mongoServer.Addr())
			Expect(err).NotTo(HaveOccurred())

			mongoDB.ContainerPort = docker.Port(mongoPort + "/tcp")
			inspectResponse.NetworkSettings.IPAddress = mongoHost
			inspectResponse.NetworkSettings.Ports[mongoDB.ContainerPort] = []docker.PortBinding{{
				HostIP:   "127.0.0.1",
				HostPort: "0",
			}}
		})
		...
		It("returns the container address", func() {
			...
			Expect(mongoDB.Address()).To(Equal(mongoServer.Addr()))
			Expect(mongoDB.ContainerAddress()).To(Equal(mongoServer.Addr()))
		})
	})
	...
	Context("when the host endpoint becomes available before the container", func() {
		...
		It("returns the host address", func() {
			...
			Expect(mongoDB.Address()).To(Equal(mongoServer.Addr()))
			Expect(mongoDB.HostAddress()).To(Equal(mongoServer.Addr()))
		})
	})
	...
	Context("when the log streams are both nil", func() {
		BeforeEach(func() {
			mongoDB.ErrorStream = nil
			mongoDB.OutputStream = nil
	...
	Context("when a name isn't provided", func() {
		...
		It("generates a unique name", func() {
			db1 := &runner.MongoDB{Client: client}
			err := db1.Start()
			Expect(err).To(HaveOccurred())
			Expect(db1.Name).ShouldNot(BeEmpty())
			Expect(db1.Name).To(HaveLen(26))

			db2 := &runner.MongoDB{Client: client}
			err = db2.Start()
			Expect(err).To(HaveOccurred())
